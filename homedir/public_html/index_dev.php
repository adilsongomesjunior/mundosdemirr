...
foreach($arquivos as $label => $basename) {
	if($label[0] != '.') {
?>
				<a href="index.php?pg=<?=$basename?>"><img src="menu_<?=$basename?>.jpg" width="147" height="61" border="0" alt="<?=$label?>" /></a>
<?php
	}
}
?>
...
function menu_arquivos($basename, $label)
{
	...
	if($result = @mysql_query('SELECT YEAR(data), MONTH(data) FROM post WHERE data <= NOW() GROUP BY 1, 2 ORDER BY 1 DESC, 2 DESC')) {
		echo '			<table border="0" cellpadding="0" cellspacing="0" align="center"><tr><td align="left" id="arquivos">'."\n";
		while(list($ano, $mes) = mysql_fetch_row($result)) {
			echo '				<a href="index.php?pg='.$basename.'&data='.$ano.'-'.$mes.'">'.$ano.' '.$GLOBALS['meses'][$mes]."</a>\n";
		}
		echo "			</td></tr></table>\n";
	}
	echo "			<hr />\n";
