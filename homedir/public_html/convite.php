...
document.write('<div id="convitePannel" style="position:absolute;display:block;top:0;left:0;width:100%;height:1px;text-align:center;opacity:1;filter:alpha(opacity=100);overflow:hidden;">');
...
document.write(			'<a href="#" style="color:#FFF;text-decoration:none;border-left:1px solid #000;border-right:1px solid #000;background:#000;font-family:Verdana;font-size:8pt;opacity:1;filter:alpha(opacity=100);" onclick="conviteFechar();">[Fechar]</a>');
document.write(		'</div>');
document.write(		'<img id="conviteImg" src="<?=$img?>" <?=$size[3]?> alt="" style="opacity:1;filter:alpha(opacity=100);" /><br />');
document.write(	'</div>');
document.write('</div>');
...
function conviteMostra(init)
{
	...
	if(obj != null && img != null) {
		...
		if(wait > 0) setTimeout("conviteMostra(" + ( !img.complete ? "true" : "false" ) + ");", wait);
	}
}
...
function conviteLinkMostra()
{
	...
	if(obj != null) {
		if(!obj.style) obj.style = obj;
		if(obj.innerHTML == "") {
			obj.innerHTML = "<div align=\"center\"><a href=\"#\" onclick=\"conviteInicia(); return false;\">Veja o convite para o lan&ccedil;amento do livro Pelo Sangue e Pela F&eacute;</a></div>\n<br />\n<img src=\"divider.gif\" width=\"545\" height=\"3\" border=\"0\" alt=\"\" align=\"center\" /><br />\n<br />\n";
		}
